...
$nativePrint = Get-Fingerprint @($appJson, (Join-Path $App 'package.json'), (Join-Path $App 'plugins'), (Join-Path $App 'modules'), (Join-Path $App '.env'), (Join-Path $App 'assets\icon.png'), (Join-Path $App 'assets\adaptive-icon.png'), (Join-Path $App 'assets\adaptive-background.png'), (Join-Path $App 'assets\splash-icon.png'), (Join-Path $Root 'pnpm-lock.yaml'), (Join-Path $App 'scripts\fix-native-modules.mjs')) "signed=$signed;playgames=$env:PLAY_GAMES_APP_ID"
