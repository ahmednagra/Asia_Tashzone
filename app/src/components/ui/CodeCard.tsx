...
export function CodeCard({ label, code, hint, actionLabel, onAction, placeholder }: { label: string; code: string; hint?: string; actionLabel?: string; onAction?: () => void; placeholder?: boolean }) {
  ...
  return (
    <GlassCard style={s.card}>
      <View style={s.main}>
        <Text style={[s.label, { color: c.textMuted }]}>{label.toUpperCase()}</Text>
        <Text accessibilityLabel={`${label}: ${spoken}`} style={[s.code, { color: placeholder ? c.textMuted : name === "dark" ? material.goldLeafHot : c.primary }]}>{code}</Text>
...
const s = StyleSheet.create({
  ...
  main: { flex: 1, gap: 4 },
  label: { fontFamily: fonts.ui.family, fontSize: 11, letterSpacing: 1.3, fontWeight: "600" },
  code: { fontFamily: fonts.display.family, fontSize: 32, fontWeight: "600", letterSpacing: 8, lineHeight: 38 },
