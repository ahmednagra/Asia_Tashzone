      ...
      {LANGS.map((l) => {
        ...
        return (
          <SelectableCard key={l.k} on={on} label={`${l.native}, ${l.en}`} onPress={() => update({ lang: l.k })}
            ...
            <Text style={{ flex: 1, color: c.text, fontSize: 17, fontFamily: fonts.ui.family, textAlign: rtl ? "right" : "left" }}>{l.native}</Text>
            <Caption>{l.en}</Caption>
            {on ? <Text style={{ color: material.goldLeaf, fontSize: 20 }} accessibilityLabel="Selected">★</Text> : null}
          </SelectableCard>
        );
      })}
...
const s = StyleSheet.create({ row: { alignItems: "center", gap: 10, minHeight: 56 } });
