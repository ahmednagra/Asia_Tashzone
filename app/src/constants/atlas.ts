...
const ATLAS_EN = {
  ...
  entries: {
    ...
    leave: { label: "Leave this match?", sub: "A bot plays your seat for the rest of the match. You can come back while the room is live." },
    paused: { label: "Table paused", sub: "The server is catching up. Every clock is frozen and restarts in full." },
    lost: { label: "Connection lost", sub: "Your seat is kept. Play resumes at your next move." },
    unreachable: { label: "Cannot reach the table", sub: "A bot is holding your seat. You can rejoin while the room is live." },
    serverUpdate: { label: "Server update", sub: "Your game continues. The match ends after this round if the update changes the rules." },
    interrupted: { label: "Match interrupted", sub: "A server update stopped the match. It counts as neither a win nor a loss." },
    updateToJoin: { label: "Update to join", sub: "This table uses newer rules than your copy of TashZone. Update, then open the link again." },
    ...
    expired: { label: "That code has expired", sub: "Rooms close once they have been idle for a while." },
    refused: { label: "You cannot join this room", sub: "" },
    removed: { label: "The host removed you", sub: "You cannot rejoin this room. Other rooms are unaffected." },
    offline: { label: "You are offline", sub: "Offline games still work. Online rooms reconnect on their own." },
    rulesChanged: { label: "Rules changed", sub: "This saved game was made under an older rule version. Start a new game to keep playing." },
    voiceOff: { label: "Voice is turned off in Parent controls", sub: "A parent can turn it back on with the PIN." },
    saveFailed: { label: "Could not save your result", sub: "It will be retried on its own. If it keeps failing, quote the code to support." },
  },
};
