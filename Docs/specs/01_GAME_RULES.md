...
| callbreak.np / callbridge.bd | FROZEN_WITH_GAPS | none for release; G-12/13/14/15/57 decided 23 Sep 2026 (§3.3) |
...
deal:
  ...
  redeal:
    ...
    bid_sum_below_8: off   # variant CB-03 (threshold 8) ; DECIDED 23 Sep 2026 (G-15)
    redeal_cap: 3          # after 3 consecutive annulments the next deal is played as dealt ; DECIDED 23 Sep 2026 (G-15)
...
play:
  ...
  obligation_default: canonical   # must-beat + must-trump-if-winning ON for every region ; relaxed rules = variant CB-01 room toggle ; DECIDED 23 Sep 2026 (G-12)
  losing_trump_when_void: any_card   # S11 ; forced losing trump stays variant CB-02 (DEFER) ; DECIDED 23 Sep 2026 (G-13)
scoring:
  unit: tenths_of_a_point (integer)   # DESIGN (EC-01)
  made: +10*call + 1*overtricks   # (+call, +0.1 per overtrick)
  failed: -10*call
match:
  rounds: 5   # CB-11 toggle: 3 / 5 / 10 / 25 / 50 / 100
  winner: highest_total
  tie: shared_placement   # standard competition ranking (1-2-2-4), no tie-break hand ; DECIDED 23 Sep 2026 (G-14)
guards: { max_actions_per_hand: 256, outcome: annul_same_dealer }   # a complete hand needs ≤ 130 ; DECIDED 23 Sep 2026 (G-57)
timers: { turn_ms: 20000, window_ms: 4000 (W-CB-1 fixed, never closes early), accessible_timers: private-room toggle doubles turn_ms (PD-07) }
...
Terminal: after the configured rounds (default 5) → MATCH_OVER; equal totals share a placement (G-14). Edge: all four fail (legal); a trump in the trick voids the led-suit must-beat obligation (§1.3); at most 3 consecutive annulments, then the deal is played as dealt (G-15); action cap 256 → annul, same dealer (G-57). **Information:** RequestRedeal intentionally reveals the requester's hand, per rule.

**Variants** (canonical defaults above; delivery: PROFILE · PRESET · TOGGLE · DEFER · REJECT)

| ID | Variant | Difference | Depends / conflicts | Delivery |
|---|---|---|---|---|
| CB-01 | Relaxed play | no must-beat; no forced trump | conflicts with canonical obligations | TOGGLE (room setting; canonical stays default everywhere, G-12 decided) |
...
| G-12 | R2 | Regional default for Callbreak forced play (must-beat, must-trump-if-winning) — **decided 23 Sep 2026: canonical ON everywhere; CB-01 toggle** | Callbreak |
| G-13 | R2 | Void with only losing trumps: may discard (S11) or must still trump — **decided: may discard (S11); CB-02 DEFER** | Callbreak |
...
| G-15 | R3 | Callbreak: prevalence of bid-sum-below-8 redeal; redeal loop cap — **decided: off by default (CB-03 toggle); cap 3 consecutive annulments** | Callbreak |
| G-16 | R3 | Court Piece: who decides to continue for a 52-court after the first 7 tricks | Court Piece |
| G-14 | R3 | Final-score ties (Callbreak totals; Badam Satti totals; Bhabhi penalty points) — **decided for Callbreak/Call Bridge: shared placement; other games open** | several |
...
### 3.3 Decisions log (implementation, 23 Sep 2026)
Taken during Phase 5 under the instruction "follow industry best practice and update the docs to avoid future conflicts". Each decision is recorded in the profile's `decisions` block (`shared/profiles/src/index.ts`), so it is part of the profile hash and the behaviour digest.

| ID | Decision | Reason |
|---|---|---|
| G-12 | Canonical obligations (must-beat, must-trump-if-winning) are the default everywhere; CB-01 relaxed play is a room toggle | Matches Pagat/S11 and the leading Callbreak apps; one default keeps ratings comparable |
| G-13 | A void seat whose trumps cannot beat the current winner may play any card; CB-02 stays DEFER | S11; forcing a losing trump adds no skill and is rare in apps |
| G-14 | Callbreak/Call Bridge final ties share the placement (1-2-2-4); no tie-break hand | Standard competition ranking; no extra hand to design or rate |
| G-15 | Bid-sum < 8 redeal off by default (CB-03 toggle, threshold 8); at most 3 consecutive annulments, then the deal is played as dealt | Prevents redeal loops without removing the traditional request |
| G-57 | Callbreak family: 256 actions per hand, outcome annul with the same dealer | A complete hand needs ≤ 130 actions including timeouts, so the cap never triggers in normal play and cannot reward stalling. Other games still need their own values |
| G-58 | Call Bridge only: 5 hands by default, CB-11 options | Aligns with Callbreak; other open-ended games still open |
| W-CB-1 | Window duration fixed at 4000 ms, identical for every viewer, never closes early; requests are sealed until close, then the requester's hand is revealed | P-16 hidden eligibility |
| CB-06 | Preset "lakdi-india" = clockwise play and clockwise dealer rotation | Regional preset rather than two separate toggles for players |
| CB-09 | Blind bid DEFERRED (needs a sealed-call window) | Not in the first slice |


---

## Addendum (Sep 2026) — TashZone v1 profiles ported into the engine

Two games from the old live TashZone now run on this engine with their v1 rules unchanged. They are
separate profiles from the catalogue entries `rung.*` and `getaway.*` above, whose open gaps (R1) still
block those spec profiles; `courtpiece.tz@1` and `bhabhi.tz@1` are the rules players already know.

**`courtpiece.tz@1` (Court Piece / Rung, FROZEN).** 4 players, partners 0+2 v 1+3, play in seat order. First
dealer seat 0; the seat after the dealer is dealt 5 cards, names trump, then everyone gets 8 more (4+4)
and the caller leads. Follow suit if able; no must-beat or must-trump. Single Sir: a hand ends when a team
has 7 tricks (`stop_at_seven`); court = 7–0. Double Sir: tricks pile up and are collected when the **same
player** wins two consecutive tricks (optional: not if both winning cards are aces), and the last trick
collects the pile; court = all 13 collected. Hand = 1 point, court = 3; first to `target_points` (3/5/7)
wins. Dealer (Pagat): if the caller's side won the dealer deals again, otherwise the next seat; after a
court the dealer moves across. Guard: 256 actions per hand, annul.

**`bhabhi.tz@1` (Bhabhi / Thulla / Get Away, FROZEN).** 3–6 players with one deck, 4–8 with two (forced
for 7–8 unless `decks` is set). Whole deck dealt; the holder of the first ace of spades leads it. Follow
suit if able; a card of another suit is a *thulla*: `immediate` (default) ends the trick at once and the
highest card of the led suit picks the trick up and leads; `finishTrick` lets everyone play first. With no
thulla the trick is discarded and the highest card leads. The first trick is always discarded (toggle).
Emptying your hand gets you away. If the power holder empties theirs: `drawFromWaste` (default: draw one
random card from the pile as it was before this trick and lead it), `drawFromNext`, `escapeIfThreePlus`
or `passToNext`. House rules: take-the-hand (before a trick, take the next active player's cards; they
get away; needs 3+ holding cards), two-player cut (a drawn lead cut with two left makes the leader the
Bhabhi). The last player holding cards is the Bhabhi. Match: `rounds` hands, or until someone is Bhabhi
`bhabhi_limit` times; ranking by fewest times Bhabhi, then lower summed finishing places. Handicap
(0/3/6 extra cards to seat 0) is for tables against bots only. Safety cap 2,000 tricks (most cards loses).

**Callbreak v1 presets** on `callbreak.np@1`: `classic` (calls 1–8, automatic redeal of any hand without a
spade, same dealer, capped at 50, +0.1 per overtrick, no redeal window), `easy-follow` (classic without
must-beat; trump obligations kept), `call-bridge-classic` (calls 2–13, no overtrick bonus). v1's Classic
follow rules equal this profile's canonical rules (must-beat, must-trump-if-winning).
