...
9. Every hand has a finite action cap with a profile-declared outcome (G-57). The behaviour digest is a function of behaviour only: it covers legal-action sets, state hashes and events of a fixed-seed simulation set, and deliberately excludes the engine build hash, so rebuilds that keep behaviour keep the digest.
...
|---|---|---|
| G-57 | action-cap outcome per game — Callbreak family decided: 256, annul with same dealer (01 §3.3) | each game's release |
| O-07 | adoption threshold for behaviour-changing engine builds | first behaviour-changing release |

---

## Addendum (Sep 2026) — generic game contract

`shared/engine/src/module.ts` defines `GameModule`, the one interface every game implements
(`callbreak`, `courtpiece`, `bhabhi`; registry in `registry.ts`, `gameOfProfile(profile_id)`). The Runtime
Shell (match server `Room`, offline `LocalTable`, `replayModule`, bots, behaviour digest) only calls this
interface. Hand lifecycle is identical for every game: `BeginHand{hand_seed}` → play → hand `DONE`
(`waitingOn` = AUTO) → next `BeginHand` … → match over (NONE). Randomness inside a hand comes only from
the hand seed via labelled tz-rng-v1 substreams (`R-CB-1/shuffle`, `R-CP-1/shuffle`, `R-BH-1/shuffle`,
`R-BH-2/draw/<trick>`). `explain(view, move)` returns the stable v1 rule codes from the viewer's own view.
Bots use `determinize` / `utility` / `medium` and a separate seeded generator (`BotRng`) that never
touches game randomness.
