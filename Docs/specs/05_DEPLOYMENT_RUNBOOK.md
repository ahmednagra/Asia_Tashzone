...
## Quick path: reinstalled VM (Sep 2026)

The VM at **141.148.193.78** was reinstalled and its database wiped. Bringing TashZone back is two commands; the
files live in `backend/deploy/` under the same names as TashZone v1.

| File | Purpose |
|---|---|
| `server-setup.sh` | Run once **on the server**: packages, host firewall (80/443 tcp, 443 udp, 7881 tcp, 7882 udp above Oracle's REJECT), Docker, 4 GB swap, `/srv/tashzone/{releases,backups}`, generated `/srv/tashzone/.env` (never overwritten), nightly backup cron, `tz` shortcut |
| `deploy.sh` | Run **from your PC** for every release: local gates, preflight (secrets present, live tables), upload, backup, release swap (keeps `repo.previous`), Caddy validation, migrations, verification from outside. `--publish-apk` publishes an APK as the in-app update |
| `rollback.sh` | Puts `repo.previous` back (code only; restore a dump if a migration ran) |
...
# 2. PC (Git Bash, repository root), every release
bash backend/deploy/deploy.sh
# appoint yourself admin moderator (player id from the app), on the server
tz exec api python -m scripts.grant_moderator <player_id> --role admin
```

Endpoints: `https://api.141-148-193-78.sslip.io`, `wss://match.141-148-193-78.sslip.io/match`,
`wss://voice.141-148-193-78.sslip.io`. Kill switch: set `ONLINE_ENABLED=false` (and optionally
`MAINTENANCE_MESSAGE_EN/UR`) in `/srv/tashzone/.env`, then `tz up -d api`; one game: `DISABLED_PROFILES=bhabhi.tz@1`;
one feature: `DISABLED_FEATURES=voice,quick_match`.
