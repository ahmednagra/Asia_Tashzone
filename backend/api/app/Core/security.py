...
from app.Core.enforcement import banned_error, is_banned
from app.Core.errors import ApiError, forbidden
from app.Models import Moderator, Player
...
def sign_player_token(secret: str, player_id: str, generation: int, days: int = 180) -> str:
...
def verify_player_token(secret: str, token: str) -> tuple[str, int] | None:
    try:
...
def authenticated_player(authorization: str | None = Header(default=None), db: Session = Depends(get_db),
